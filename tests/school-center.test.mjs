...
test("new school center supports search, filters, selected conditions, and decision actions", async () => {
  ...
  for (const label of ["搜尋校名、科名、群科、縣市、學校代碼", "就學區", "學制", "公私立", "地區", "有招生名額", "有歷年參考資料", "已選條件", "清除條件", "加入規劃", "加入比較", "查看官方網站"]) {
    assert.match(explorer, new RegExp(label));
  }
  assert.match(explorer, /hasQuota/);
  assert.match(explorer, /hasHistoricalData/);
  assert.match(explorer, /compare/);
  assert.match(explorer, /資料狀態/);
});

test("school details use the six decision sections and Google Maps", async () => {
  const [page, actions] = await Promise.all([
    read("app/schools/[district]/[code]/page.tsx"),
    read("components/school-decision-actions.tsx"),
  ]);

  for (const label of ["一眼看懂", "學習內容", "招生資訊", "歷年參考", "生活條件", "決策操作", "歷年參考區間", "適合什麼樣的學生", "住宿／交通資訊", "Google Maps"]) {
    assert.match(page, new RegExp(label));
  }
  for (const label of ["加入挑戰", "加入適中", "加入穩定", "加備註", "加入比較"]) {
    assert.match(actions, new RegExp(label));
  }
  assert.match(page, /maps\/search/);
});
