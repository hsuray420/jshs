...
const names = new Set();
for (const district of Object.keys(metadata.districts)) {
  const suffix = district === "tp" ? "_tp" : district === "taoyuan-lienchiang" ? "_tl" : "";
  const csv = await readFile(resolve(root, `public/it_hs/${district}/schools${suffix}.csv`), "utf8");
  const rows = parseCsvRows(csv); const headers = rows[0].map((item) => item.replace(/^\uFEFF/, "").trim()); const departmentsIndex = headers.indexOf("科系與名額");
  for (const row of rows.slice(1)) for (const item of (row[departmentsIndex] || "").split("；")) { const name = item.split(":")[0]?.trim(); if (name) names.add(name); }
}
