...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> { const article = getNewsArticle((await params).slug); if (!article) return { title: "找不到文章｜全國國中生升學資訊網", robots: { index: false, follow: false } }; return { title: `${article.title}｜升學情報`, description: article.description, alternates: { canonical: `/news/${article.slug}` } }; }
