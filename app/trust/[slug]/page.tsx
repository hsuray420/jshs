...
function AboutContent() { return <div className="grid gap-5 text-sm leading-7 text-slate-600"><p className="text-lg font-bold text-[var(--jshs-primary)]">讓每一個升學選擇，都有資料可以理解、有來源可以確認。</p><p>全國國中生升學資訊網 是獨立的升學資訊整理平台，服務學生、家長與老師。本站不是教育部、各招生委員會、任何學校或補習班的官方網站，也不代表上述單位做出資格或錄取判定。</p><section><h2>資料哪裡來？</h2><p>招生規則、簡章、日期與正式平台會優先連回官方來源；學校目錄、生活資料與工具結果則標示 JSHS 整理、計算、推估或社群資料，並保留資料年度與最後確認時間。</p></section><section><h2>誰維護？</h2><p>平台維護者負責整理來源、修正錯誤、更新功能與公開更新紀錄。若你發現問題，請使用資料回報，讓每筆修正都有可追溯的處理入口。</p></section><section><h2>我們不收什麼？</h2><p>能不收的資料就不收。查詢與試算主要可匿名完成；請不要在備註、回報或分享中填寫身分證字號、完整地址、電話或其他不必要的敏感資料。</p></section></div>; }
