...
export default async function AdminContentPage({ searchParams }: { searchParams: Promise<{ edit?: string; type?: string; updated?: string; preview?: string }> }) {
  ...
  return <main className="admin-shell">
    ...
    {params.updated ? <section className="admin-flash">{params.updated === "published" ? "內容已發布，前台會讀取最新版本。" : params.updated === "unpublished" ? "內容已退回草稿。" : params.updated === "draft" ? "草稿已儲存。" : "內容格式不正確，尚未更新。"}</section> : null}
    ...
    <section className="admin-grid admin-grid-3">
      ...
      <section className="admin-panel admin-content-editor"><div className="admin-section-head"><div><p className="admin-eyebrow">Editor</p><h2>{editing ? "編輯內容" : "新增內容"}</h2></div>{editing ? <span className={editing.status === "published" ? "admin-badge ok" : "admin-badge warn"}>{editing.status === "published" ? "已發布" : "草稿"}</span> : null}</div><p className="admin-muted">body JSON 只放內容資料，不放程式碼。先儲存草稿，確認後再發布。</p><form className="admin-content-form" action="/api/admin/content" method="post"><input type="hidden" name="action" value="save" />{editing ? <input type="hidden" name="id" value={editing.id} /> : null}<label>內容類型<select name="content_type" defaultValue={contentType}>{CONTENT_TYPES.map((type) => <option key={type} value={type}>{typeLabels[type]}</option>)}</select></label><label>識別網址 slug<input name="slug" defaultValue={editing?.slug || ""} placeholder="例如：序位" maxLength={120} required /></label><label>標題<input name="title" defaultValue={editing?.title || ""} maxLength={160} required /></label><label>摘要／前台說明<textarea name="summary" defaultValue={editing?.summary || ""} rows={3} maxLength={1000} /></label><label>內容資料 JSON<textarea name="body_json" defaultValue={editing?.body_json || defaultBody(contentType)} rows={12} spellCheck={false} required /></label><label>儲存狀態<select name="status" defaultValue={editing?.status || "draft"}><option value="draft">草稿</option><option value="published">儲存並發布</option></select></label><div className="admin-row-actions"><button className="admin-button" type="submit">儲存內容</button>{editing ? <Link className="admin-button admin-button-secondary" href={`/admin/content?preview=${editing.id}`} target="_blank">預覽</Link> : null}</div></form>{editing ? <div className="admin-content-actions"><form action="/api/admin/content" method="post"><input type="hidden" name="action" value={editing.status === "published" ? "unpublish" : "publish"} /><input type="hidden" name="id" value={editing.id} /><button type="submit">{editing.status === "published" ? "退回草稿" : "發布這筆內容"}</button></form><small>更新者：{editing.updated_by} · {formatDate(editing.updated_at)}</small></div> : null}</section>
    </section>
