    ...
    <main className="min-h-screen jshs-page-shell">
      ...
      <header className="jshs-hero-section">
        <div className="mx-auto w-[min(1120px,calc(100%-32px))] py-12 md:py-18">
          <nav aria-label="麵包屑" className="flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500"><Link href="/">首頁</Link><span aria-hidden="true">/</span><Link href="/schools">找校科中心</Link><span aria-hidden="true">/</span><Link href={`/schools?district=${school.districtCode}`}>{school.districtLabel}</Link><span aria-hidden="true">/</span><span className="text-[var(--jshs-primary)]">{school.code}</span></nav>
      ...
      <nav aria-label="學校詳情分頁" className="border-b border-slate-200 bg-white"><div className="mx-auto flex w-[min(1120px,calc(100%-32px))] gap-2 overflow-x-auto py-4">{[["overview", "一眼看懂"], ["learning", "學習內容"], ["admission", "招生資訊"], ["history", "歷年參考"], ["life", "生活條件"], ["decision", "決策操作"]].map(([id, label]) => <a key={id} href={`#${id}`} className="shrink-0 px-4 py-2 text-sm jshs-button-secondary">{label}</a>)}</div></nav>

      <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:py-14">
        <div className="space-y-4">
          <SchoolSection id="overview" title="一眼看懂" eyebrow="OVERVIEW" open><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Fact label="學校類型" value={school.program || "待確認"} /><Fact label="地區" value={[school.city, school.area].filter(Boolean).join(" ") || "待確認"} /><Fact label="公私立" value={school.ownership || "待確認"} /><Fact label="目前資料年度" value={`${school.academicYear} 學年度`} /></div><p className="mt-5 rounded-2xl bg-[var(--jshs-muted-surface)] p-4 text-sm leading-7 text-slate-700">資料狀態：{school.dataStatus === "ready" ? "本區資料已整理，可作為初步比較" : "本區為參考資料，請等待或回查當年度正式公告"}。</p></SchoolSection>

          <SchoolSection id="learning" title="學習內容" eyebrow="LEARNING"><div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div><h3 className="font-black">科別與課程方向</h3>{school.departments.length ? <ul className="mt-4 grid gap-3 sm:grid-cols-2">{school.departments.map((department) => <li key={`${department.name}-${department.quota}`} className="rounded-2xl bg-[var(--jshs-muted-surface)] p-4"><strong className="text-[var(--jshs-primary)]">{department.name}</strong><span className="mt-2 block text-sm text-slate-600">名額：{department.quota === null ? "待公告" : department.quota} · {department.audience || "招生對象依簡章"}</span></li>)}</ul> : <p className="mt-4 leading-7 jshs-muted-copy">科別尚未整理，請查看學校官方公告。</p>}</div><div className="rounded-2xl border border-[var(--jshs-border)] p-5"><h3 className="font-black">實習／專題</h3><p className="mt-3 text-sm leading-7 text-slate-600">{school.specialPrograms || "目前資料未提供，請從學校課程與招生簡章確認。"}</p><h3 className="mt-6 font-black">適合什麼樣的學生</h3><p className="mt-3 text-sm leading-7 text-slate-600">適合願意投入{school.program || "這個學制"}學習方式，並願意進一步比較科別內容、通勤與未來路徑的學生。</p></div></div></SchoolSection>

          <SchoolSection id="admission" title="招生資訊" eyebrow="ADMISSION"><div className="grid gap-4 sm:grid-cols-2"><Fact label="招生名額" value={school.quota || (school.hasQuota ? "依科別資料彙整" : "待公告")} /><Fact label="招生管道" value={school.admissionTrack || `${school.districtLabel}免試入學`} /><Fact label="正式簡章" value="請以本區官方入口最新版本為準" /><Fact label="資料來源" value={school.sourceName} /></div><div className="mt-6 flex flex-wrap gap-3"><a className="px-4 py-3 text-sm jshs-button-primary" href={school.sourceUrl} target="_blank" rel="noreferrer">查看正式招生來源 ↗</a>{school.website ? <a className="px-4 py-3 text-sm jshs-button-secondary" href={school.website} target="_blank" rel="noreferrer">查看學校官方網站 ↗</a> : null}</div></SchoolSection>

          <SchoolSection id="history" title="歷年參考" eyebrow="HISTORICAL REFERENCE"><h3 className="font-black">歷年參考區間</h3><p className="mt-3 text-2xl font-black text-amber-950">{school.referenceScore || "目前沒有可公開的歷年參考資料"}</p><p className="mt-2 text-sm font-bold text-amber-800">參考年度：{school.scoreYear || "未標示"}</p><p className="mt-5 rounded-2xl bg-amber-50 p-5 text-sm leading-7 text-amber-950">這裡只呈現歷年資料形成的參考區間，不是本年度錄取結果。名額、報名人數、志願序與同分比序都可能改變，正式判斷請回到當年度簡章。</p></SchoolSection>

          <SchoolSection id="life" title="生活條件" eyebrow="LIFE & COMMUTE"><div className="grid gap-5 sm:grid-cols-[1fr_auto]"><div><dl className="grid gap-4 text-sm sm:grid-cols-[120px_1fr]"><dt className="font-black text-slate-500">地址</dt><dd className="font-bold leading-6">{school.address || "資料未提供"}</dd><dt className="font-black text-slate-500">電話</dt><dd className="font-bold leading-6">{school.phone || "資料未提供"}</dd><dt className="font-black text-slate-500">住宿／交通資訊</dt><dd className="font-bold leading-6">目前未整理住宿與通勤路線，請以學校官方資料及家庭實際路線確認。</dd></dl></div><a className="h-fit px-4 py-3 text-sm jshs-button-secondary" href={mapsUrl} target="_blank" rel="noreferrer">在 Google Maps 查看 ↗</a></div><p className="mt-5 text-xs leading-6 text-slate-500">地址來源：{school.sourceName}；地圖連結依學校名稱與地址產生，出發前請再次核對。</p></SchoolSection>
