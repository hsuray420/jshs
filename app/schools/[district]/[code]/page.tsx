    ...
    <main className="min-h-screen jshs-page-shell">
      ...
      <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:py-14">
        <div className="space-y-4">
          ...
          <SchoolSection id="life" title="生活條件" eyebrow="LIFE & COMMUTE"><div><dl className="grid gap-4 text-sm sm:grid-cols-[120px_1fr]"><dt className="font-black text-slate-500">地址</dt><dd className="font-bold leading-6">{school.address || "資料未提供"}</dd><dt className="font-black text-slate-500">電話</dt><dd className="font-bold leading-6">{school.phone || "資料未提供"}</dd><dt className="font-black text-slate-500">交通方式</dt><dd className="font-bold leading-6">{school.transport || "資料未提供，請以學校官方資料確認"}</dd><dt className="font-black text-slate-500">通勤資訊</dt><dd className="font-bold leading-6">{school.commuteInfo || "資料未提供，請用家庭實際出發地確認"}</dd><dt className="font-black text-slate-500">住宿資訊</dt><dd className="font-bold leading-6">{school.boardingInfo || "資料未提供，請以學校官方資料確認"}</dd><dt className="font-black text-slate-500">生活資料來源</dt><dd className="font-bold leading-6">{school.lifeSource || "資料未提供，請以學校官方資料確認"}</dd></dl></div><p className="mt-5 text-xs leading-6 text-slate-500">地址來源：{school.sourceName}；{school.lifeSource ? `生活資料來源：${school.lifeSource}；` : "生活資料尚未提供；"}</p></SchoolSection>
...
function MissingField({ label }: { label: string }) {
  return <p className="mt-3 rounded-2xl border border-dashed border-[var(--jshs-border)] bg-white p-4 text-sm leading-6 text-slate-500">{label}：資料尚未提供；目前不推估。</p>;
}
