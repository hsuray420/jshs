...
export default async function SchoolCompareRoute({ searchParams }: { searchParams: Promise<{ district?: string }> }) {
  ...
  return <main className="min-h-screen jshs-page-shell"><SiteHeader activeHref="/schools" /><FeatureHero theme="schools" eyebrow="找學校 · 學校比較" title="並排比較你正在考慮的學校" description="選擇 2 至 4 所學校或校科，比較招生資料、地點與已提供資訊。" illustration="school-compare" /><DistrictGate initialDistrict={params.district}><SchoolComparisonExplorer districtOptions={schoolDistrictOptions} initialDistrict={params.district || "all"} /></DistrictGate><SiteFooter /></main>;
}
