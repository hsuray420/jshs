    ...
    <main className="min-h-screen jshs-page-shell">
      ...
      <header className="jshs-hero-section">
        <div className="mx-auto w-[min(1120px,calc(100%-32px))] py-12 md:py-18">
          <nav aria-label="麵包屑" className="flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500">
            ...
            <span className="text-[var(--jshs-primary)]">{school.code}</span>
          </nav>
          <div className="mt-9 flex flex-wrap gap-2 text-xs font-black">
            <span className="jshs-chip">115 學年度</span>
            <span className="jshs-chip">中投區</span>
            {[school.ownership, school.program, school.gender].filter(Boolean).map((label) => (
              <span key={label} className="jshs-chip">{label}</span>
            ))}
          </div>
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 jshs-muted-copy">整理校科、招生名額、地址與聯絡方式，協助你先確認選項，再接續比較、試算與志願規劃。</p>
        </div>
      </header>
      ...
      <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start lg:py-14">
        <div className="space-y-8">
          ...
          <section aria-labelledby="departments" className="p-6 md:p-8 jshs-surface-card">
            <p className="jshs-eyebrow">PROGRAMS & QUOTAS</p>
            <h2 id="departments" className="mt-3 text-3xl font-black tracking-[-.04em]">115 學年度科別與名額</h2>
            <p className="mt-3 leading-7 jshs-muted-copy">以下為中投區免試入學資料中的一般招生名額。實際名額與資格仍以當年度正式簡章及學校公告為準。</p>
            {school.departments.length ? (
              <div className="mt-7 grid gap-3 sm:grid-cols-2">
                {school.departments.map((department) => (
                  <article key={`${department.name}-${department.quota}`} className="rounded-2xl border border-[var(--jshs-border)] bg-[var(--jshs-muted-surface)] p-5">
                    <h3 className="text-lg font-black text-[var(--jshs-primary)]">{department.name}</h3>
          ...
          <section aria-labelledby="contact" className="p-6 md:p-8 jshs-surface-card">
            <p className="jshs-eyebrow">CONTACT</p>
            ...
            {school.website ? <a className="mt-7 inline-flex px-5 py-3 text-sm jshs-button-secondary" href={school.website} target="_blank" rel="noreferrer">前往學校官網 ↗</a> : null}
          </section>

          {school.referenceScore ? (
            <section aria-labelledby="score-reference" className="p-6 md:p-8 jshs-surface-card">
          ...
          <section aria-labelledby="official-source" className="p-6 md:p-8 jshs-surface-card">
            <p className="jshs-eyebrow">FACT CHECK</p>
            <h2 id="official-source" className="mt-3 text-2xl font-black">官方資料來源</h2>
            <p className="mt-4 leading-7 jshs-muted-copy">{school.sourceNote || `${district.academicYear} 學年度中投區免試入學招生資料。`}</p>
            <div className="mt-5 flex flex-wrap gap-3 text-sm font-black">
              <a className="px-4 py-3 text-sm jshs-button-secondary" href={district.sourceUrl} target="_blank" rel="noreferrer">{district.sourceName} ↗</a>
              <span className="jshs-chip">本站更新：{district.updatedAt}</span>
            </div>
          </section>
        </div>

        <aside className="space-y-5 lg:sticky lg:top-24">
          <section className="p-6 jshs-surface-card">
            <p className="jshs-eyebrow">下一步</p>
            <h2 className="mt-3 text-2xl font-black">把這所學校放進你的決策清單</h2>
            <p className="mt-3 text-sm leading-7 jshs-muted-copy">先收藏，再搭配積分與其他校科比較，不必一次決定志願順序。</p>
            ...
            <div className="mt-4 grid gap-2">
              <a className="px-4 py-3 text-sm jshs-button-secondary" href="/it_hs/guide.htm?district=ct#calculator">開始中投區積分試算 →</a>
              <a className="px-4 py-3 text-sm jshs-button-secondary" href="/it_hs/guide.htm?district=ct#analysis">進入志願規劃台 →</a>
              <Link className="px-4 py-3 text-sm jshs-button-secondary" href={`/schools?district=ct&q=${encodeURIComponent(school.code)}`}>返回中投區校科查詢 →</Link>
            </div>
          </section>

          <section className="p-6 jshs-surface-card">
            <p className="jshs-eyebrow">同區延伸探索</p>
            <div className="mt-4 grid gap-2">
              {relatedSchools.map((related) => (
                <Link key={related.code} href={`/schools/ct/${related.code}`} className="rounded-2xl border border-[var(--jshs-border)] p-4 hover:bg-[var(--jshs-muted-surface)]">
                  <b className="block text-sm text-[var(--jshs-primary)]">{related.name}</b>
...
function FactCard({ label, value }: { label: string; value: string }) {
  return <article className="p-5 jshs-surface-card"><span className="text-xs font-black tracking-[.1em] text-slate-400">{label}</span><strong className="mt-2 block text-lg leading-7 text-[var(--jshs-primary)]">{value}</strong></article>;
}
