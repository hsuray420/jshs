  ...
  return (
    <main className="min-h-screen bg-[#f6f9fd] text-[#14213d]">
      ...
      <section aria-labelledby="start-title" className="mx-auto w-[min(1160px,calc(100%-32px))] py-16 md:py-20">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4"><StartCard number="01" title="還不懂規則" body="先看會考、免試入學與志願策略。" href="/news" action="閱讀升學情報" /><StartCard number="02" title="想找學校" body="從地區、校科與五專路徑開始探索。" href="/schools" action="進入新版查校" /><StartCard number="03" title="需要試算比較" body="查看積分、落點與比較工具。" href="/tools" action="進入新版試算" /><StartCard number="04" title="已經有候選清單" body="整理收藏、志願、日期與待辦。" href="/planner" action="打開雲端規劃" /></div>
      </section>
      ...
      <section id="tools" aria-labelledby="tools-title" className="border-y border-slate-200 bg-white py-16 md:py-20">
        <div className="mx-auto w-[min(1160px,calc(100%-32px))]"><SectionHeading eyebrow="熱門升學工具" id="tools-title" title="把查到的資料，變成可以比較的選項。" body="工具不替你保證錄取；它協助整理積分、風險、通勤與志願結構。" /><div className="mt-8 grid gap-4 lg:grid-cols-3"><ToolCard title="積分與落點" body="依正確就學區和學年度使用新版試算功能。" href="/tools" /><ToolCard title="校科比較" body="直接進入新版學校查詢，將候選校科存入 Cloudflare。" href="/schools" /><ToolCard title="志願與待辦" body="把收藏與志願規劃保存於 Cloudflare D1。" href="/planner" /></div></div>
      </section>
      ...
      <section id="districts" aria-labelledby="districts-title" className="mx-auto w-[min(1160px,calc(100%-32px))] py-16 md:py-20">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{districts.map(([code, district]) => <a key={code} className="group rounded-3xl border border-slate-200 bg-white p-6 transition hover:-translate-y-1 hover:border-blue-300 hover:shadow-xl hover:shadow-blue-950/10" href={`/schools?district=${code}`}><div className="flex items-start justify-between gap-3"><span className="text-xs font-extrabold tracking-[.12em] text-[#2868d7]">{code.toUpperCase()}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500">{district.academicYear} 學年度</span></div><strong className="mt-4 block text-2xl">{district.label}</strong><small className="mt-1 block min-h-10 text-sm text-slate-500">{district.areas}</small><b className="mt-5 flex items-center justify-between text-sm text-[#173d78]">進入新版學校查詢 <span className="transition group-hover:translate-x-1">→</span></b></a>)}</div>
