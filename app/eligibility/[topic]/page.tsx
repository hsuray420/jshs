...
export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> { const { topic } = await params; const page = pages[topic]; return page ? { title: `${page.title}｜特殊資格`, description: page.description, alternates: { canonical: `/eligibility/${topic}` } } : {}; }
export default async function EligibilityTopicPage({ params }: { params: Promise<{ topic: string }> }) { const { topic } = await params; const page = pages[topic]; if (!page) return null; return <><SiteHeader activeHref="/eligibility" /><IndependentSupportPage page={page} /><SiteFooter /></>; }
