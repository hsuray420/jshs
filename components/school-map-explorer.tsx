...
export function SchoolMapExplorer({ districtOptions, initialDistrict = "all" }: { districtOptions: readonly { code: string; label: string }[]; initialDistrict?: string }) {
  ...
  const commuteRows = useMemo(() => selectedSchoolRecords.map((school) => {
    ...
    return { school, distance, minutes: Math.max(1, Math.round(distance * settings.roadFactor / settings.speed * 60)) };
  }).filter((row): row is { school: MapRecord; distance: number; minutes: number } => Boolean(row)).sort((left, right) => left.minutes - right.minutes), [coordinates, homeCoordinate, selectedSchoolRecords, travelMode]);
