...
const primaryNavigation = [
  { label: "找學校", href: "/schools", tone: "school" },
  { label: "算成績", href: "/tools", tone: "score" },
  { label: "我的志願", href: "/planner", tone: "planner" },
  { label: "升學指南", href: "/schedule", tone: "guide" },
  { label: "官方資訊", href: "/admission-guides", tone: "official" },
  { label: "資料與信任", href: "/trust", tone: "trust" },
] as const;

const quickActions = [
  { label: "找學校", href: "/schools", icon: "school", tone: "school", description: "搜尋學校、科系與歷年資料" },
  { label: "算成績", href: "/tools", icon: "calculator", tone: "score", description: "依就學區規則完成積分試算" },
  { label: "升學日程", href: "/schedule", icon: "calendar", tone: "guide", description: "掌握重要日期與升學待辦" },
  { label: "我的志願", href: "/planner", icon: "planner", tone: "planner", description: "整理候選校科與志願順序" },
] as const satisfies ReadonlyArray<{ label: string; href: string; icon: SiteIconName; tone: "school" | "score" | "planner" | "guide"; description: string }>;

const mobileNavigation = [
  { label: "首頁", href: "/", icon: "home", tone: "school" },
  quickActions[0],
  quickActions[1],
  quickActions[3],
] as const;
  ...
  return <>
    <header className="sticky top-0 z-40 w-full border-b border-[var(--jshs-border)] bg-white jshs-site-header">
      <div className="jshs-header-inner">
        ...
        <nav aria-label="主要導覽" className="jshs-desktop-nav">{primaryNavigation.map((item) => <Link key={item.href} href={item.href} aria-current={activeHref === item.href ? "page" : undefined} className={`is-${item.tone} ${activeHref === item.href ? "is-active" : ""}`}>{item.label}</Link>)}</nav>
