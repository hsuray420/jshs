...
export function SchoolExplorer({ schools: initialSchools = [], initialFilters = {} }: { schools?: readonly SchoolSearchIndexEntry[]; initialFilters?: SchoolExplorerFilters }) {
  ...
  return <div className={`sv-root${hasIntent ? " is-search-mode" : ""}`}><header className="sv-hero"><div className="sv-container"><p className="sv-eyebrow">JSHS.CC · 官方資料查校</p><h1>找到適合你的學校</h1><p className="sv-hero-description">從官方資料開始，認識每一所高中。</p><SearchBox value={queryInput} onChange={setQueryInput} /><div className="sv-district-control"><span className="sv-district-label"><SiteIcon name="school" size={18} />目前就學區</span><select value={district} onChange={(event) => changeDistrict(event.target.value)} aria-label="選擇目前就學區">{regions.map((item) => <option key={item.id} value={item.id} disabled={item.schoolDataStatus !== "available"}>{item.name}{item.schoolDataStatus !== "available" ? "（資料準備中）" : ""}</option>)}</select><small>依你的位置初步建議，如需調整可隨時切換</small></div></div></header><div className="sv-container sv-shortcuts"><Link href="#popular"><SiteIcon name="school" size={20} /><span>熱門學校</span></Link><Link href="#filters"><SiteIcon name="shield" size={20} /><span>學校類型</span></Link><Link href="#filters"><SiteIcon name="search" size={20} /><span>縣市</span></Link><Link href="#filters"><SiteIcon name="knowledge" size={20} /><span>科別</span></Link><button type="button" onClick={() => drawer.current?.showModal()}><SiteIcon name="more" size={20} /><span>更多</span></button></div>{!hasIntent ? <section className="sv-container sv-popular-section" id="popular"><div className="sv-section-heading"><div><h2>{region?.name || "目前就學區"}熱門學校</h2><p>這些是許多同學關注的學校，從這裡開始探索。</p></div><a href="#explore">查看全部 <SiteIcon name="chevron-right" size={15} /></a></div>{popularSchools.length ? <div className="sv-school-grid sv-popular-grid">{popularSchools.map((school) => <SchoolCard key={school.code} school={school} compact />)}</div> : <div className="sv-inline-empty">目前沒有已設定的熱門學校，請從下方完整清單探索。</div>}</section> : null}<section className="sv-container sv-explore-section" id="explore"><div className="sv-section-heading"><div><h2>探索{region?.name || "目前就學區"}所有學校</h2><p>使用篩選條件，快速找到符合你需求的學校。</p></div></div><div className="sv-filter-row" id="filters"><button type="button" onClick={() => drawer.current?.showModal()}><SiteIcon name="shield" size={17} />篩選條件{active ? ` (${active})` : ""}</button><label>排序方式<select value={sort} onChange={(event) => setSort(event.target.value)}><option value="name">熱門程度／名稱</option><option value="city">縣市</option><option value="ownership">公私立</option></select></label></div><div className="sv-result-meta"><span>共 <strong>{result.length}</strong> 所學校</span><span className="sv-availability-note">資料範圍依目前可用的官方區域資料</span></div>{result.length ? <div className="sv-school-grid">{result.slice(0, 24).map((school) => <SchoolCard key={school.code} school={school} selected={compareCodes.includes(school.code)} onCompare={toggleCompare} />)}</div> : <div className="sv-empty"><h2>找不到符合條件的學校</h2><p>試試較短的關鍵字，或減少篩選條件。</p><button className="sv-button" type="button" onClick={() => { setQueryInput(""); clear(); }}>重新搜尋</button></div>}</section><p className="sv-data-note">學校基本資料可先瀏覽；招生、交通與住宿資訊依學校與資料來源分別顯示，尚未提供的欄位不會讓學校從結果消失。</p>{compareCodes.length ? <div className="sv-compare-bar"><span>已選 {compareCodes.length} 所學校</span><Link href={`/schools/compare?schools=${compareCodes.join(",")}`}>開始比較 <SiteIcon name="chevron-right" size={15} /></Link></div> : null}<dialog ref={drawer} className="sv-drawer"><div className="sv-drawer-head"><strong>調整搜尋條件</strong><button type="button" onClick={() => drawer.current?.close()} aria-label="關閉更多篩選"><SiteIcon name="close" size={18} /></button></div><FilterPanel prefix="mobile" schools={scopedSchools} filters={filters} update={update} clear={clear} /><button className="sv-button sv-primary" type="button" onClick={() => drawer.current?.close()}>查看 {result.length} 所學校</button></dialog></div>;
}
