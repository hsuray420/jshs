  ...
  return <section id="admission-path-finder" aria-labelledby="admission-path-title" className="mx-auto w-[min(1160px,calc(100%-32px))] py-8 md:py-12">
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <div className="p-6 md:p-8 jshs-surface-card">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="jshs-eyebrow">升學路徑與資格判定中心</p><h1 id="admission-path-title" className="mt-2 max-w-3xl">回答幾個問題，找出可能適用的升學方式。</h1><p className="mt-3 max-w-2xl text-base leading-7 jshs-muted-copy">系統依目前已核對的規則資料整理結果；資料不足的路徑會標示需要人工／官方確認。</p></div><span className="jshs-chip">115 學年度</span></div>
        {dataError ? <div className="mt-5 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-950" role="alert">{dataError}</div> : null}
...
function ResultView({ result, zone, onReset }: { result: AdmissionPathResult; zone: string; onReset: () => void }) {
  if (result.error) return <section className="mx-auto w-[min(1160px,calc(100%-32px))] py-10"><div className="p-6 jshs-surface-card" role="alert"><p className="jshs-eyebrow">檢測無法完成</p><h1 className="mt-2">{result.error.message}</h1><button type="button" onClick={onReset} className="mt-5 px-4 py-3 text-sm jshs-button-primary">重新檢測</button></div></section>;
...
function ActionLink({ label, route }: { label: string; route: AdmissionPathRoute }) { const href = route.routeId === "general-no-exam" ? "/tools" : route.routeId === "cross-zone" ? "/eligibility/cross-district" : route.routeId === "special-identity" ? "/eligibility/extra-quota" : "/trust/sources"; return <a href={href} className="rounded-xl bg-[var(--jshs-brand-tint)] px-4 py-3 text-sm font-black text-[var(--jshs-primary)]">{label} ↗</a>; }
