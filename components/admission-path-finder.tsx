...
export function AdmissionPathFinder() {
  ...
  useEffect(() => {
    ...
    fetch("/data/school-search-index.json", { headers: { accept: "application/json" } })
      .then((response) => { if (!response.ok) throw new Error("school_directory_unavailable"); return response.json() as Promise<{ schools?: SchoolSearchIndexEntry[] }>; })
  ...
  return <section id="admission-path-finder" aria-labelledby="admission-path-title" className="mx-auto w-[min(1160px,calc(100%-32px))] py-8 md:py-12">
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <div className="p-6 md:p-8 jshs-surface-card">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="jshs-eyebrow">升學路徑與資格判定中心</p><h2 id="admission-path-title" className="mt-2 max-w-3xl">回答幾個問題，找出可能適用的升學方式。</h2><p className="mt-3 max-w-2xl text-base leading-7 jshs-muted-copy">系統依目前已核對的規則資料整理結果；資料不足的路徑會標示需要人工／官方確認。</p></div><label className="grid gap-1 text-sm font-black text-[var(--jshs-primary)]">學年度<select aria-label="選擇學年度" value={state.academicYear} onChange={(event) => update({ academicYear: event.target.value })} className="min-w-32"><option value="">請選擇</option>{academicYears.map((year) => <option key={year.value} value={year.value}>{year.label}</option>)}</select></label></div>
...
function WizardStep({ kind, state, update, chooseCounty, toggleIdentity, toggleNeed, countyOptions, inferredZones, selectedZone, schoolQuery, setSchoolQuery, schools }: { kind: string; state: FinderState; update: (patch: Partial<FinderState>) => void; chooseCounty: (county: string) => void; toggleIdentity: (value: Identity) => void; toggleNeed: (value: SpecialNeed) => void; countyOptions: readonly string[]; inferredZones: readonly { code: string; label: string }[]; selectedZone?: string; schoolQuery: string; setSchoolQuery: (value: string) => void; schools: readonly SchoolSearchIndexEntry[] }) {
  ...
  if (kind === "school") return <StepShell eyebrow="Step 2" title="你目前／原就讀學校在哪裡？" help="縣市可以協助系統先推測免試就學區；學校名稱只需從選單選取，不用自行輸入。"><label className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">縣市<select value={state.schoolCounty} onChange={(event) => chooseCounty(event.target.value)}><option value="">請選擇縣市</option>{countyOptions.map((county) => <option key={county} value={county}>{county}</option>)}</select></label><label className="mt-5 grid gap-2 text-sm font-black text-[var(--jshs-primary)]">搜尋學校（選填）<input value={schoolQuery} onChange={(event) => setSchoolQuery(event.target.value)} placeholder="輸入關鍵字篩選學校選單" aria-describedby="school-select-help" /><select className="mt-1" value={state.schoolCode} onChange={(event) => update({ schoolCode: event.target.value })}><option value="">先不選學校</option>{schools.map((school) => <option key={`${school.admissionDistricts[0] || "school"}-${school.code}`} value={school.code}>{school.name}</option>)}</select><span id="school-select-help" className="font-normal text-sm leading-6 text-[var(--jshs-muted)]">學校選單載入失敗時，可以先用縣市完成檢測，結果會保守標示。</span></label></StepShell>;
