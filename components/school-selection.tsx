...
export function SchoolSelection({ schools, selected, onToggle, initialDistrict = "", maximum = 4 }: { schools: readonly SchoolSearchIndexEntry[]; selected: readonly string[]; onToggle: (code: string) => void; initialDistrict?: string; maximum?: number }) {
  const [query, setQuery] = useState("");
  const [district, setDistrict] = useState(initialDistrict);
  const districts = useMemo(() => [...new Set(schools.flatMap(s => s.admissionDistricts))].sort(), [schools]);
  const normalizedQuery = query.normalize("NFKC").replaceAll("台", "臺").replace(/\s+/g, "").toLowerCase();
  const matches = useMemo(() => schools.filter(s => (!district || s.admissionDistricts.includes(district)) && (!normalizedQuery || s.normalizedSearchText.includes(normalizedQuery))), [schools, district, normalizedQuery]);
