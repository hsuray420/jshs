...
export function SchoolComparisonExplorer({ initialDistrict }: { initialDistrict?: string }) {
  ...
  const rows: readonly [string, (s: SchoolDetail) => string][] = [
    ...
    ['資優班/特色班', s => s.features], ['課程方向', s => s.courseDirection], ['實習／專題', s => s.project], ['校車／專車資訊', s => s.transport], ['通勤資訊', s => s.commute], ['住宿資訊', s => s.lodging],
  ];
  return <section className="mx-auto w-[min(1180px,calc(100%-32px))] py-8">{loading ? <p>正在載入學校搜尋索引…</p> : error ? <p role="status">{error}</p> : <SchoolSelection schools={schools} selected={selected} onToggle={toggle} initialDistrict={initialDistrict} />}{detailsLoading ? <p className="mt-6 rounded-xl border border-dashed p-5">正在載入選取學校詳細資料…</p> : chosen.length < 2 ? <p className="mt-6 rounded-xl border border-dashed p-5">請選擇 2 至 4 所學校，查看完整比較。</p> : <section className="mt-8"><h2 className="text-2xl font-bold">學校比較 · 115 學年度</h2><p className="mt-2 text-sm">各招生區名額分別列示，不跨區加總；進修部為獨立招生單位。</p><div className="mt-4 grid min-w-0 gap-4 md:grid-cols-2">{chosen.map(s => <article className="min-w-0 rounded-2xl border p-4 md:p-6" key={s.code}><h3 className="text-xl font-bold">{s.name}</h3><Link className="inline-flex min-h-11 items-center text-blue-700" href={`/schools/${s.code}`}>查看學校與官方來源 →</Link><dl className="divide-y">{rows.map(([label, value]) => <div className="py-3" key={label}><dt className="font-bold">{label}</dt><dd className="mt-1 whitespace-pre-wrap break-words text-sm leading-7">{field(value(s))}</dd></div>)}</dl></article>)}</div></section>}</section>;
}
