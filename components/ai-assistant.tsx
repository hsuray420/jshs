...
function ChatWorkspace({ mode, onClose, initialConversationId = "" }: { mode: WorkspaceMode; onClose?: () => void; initialConversationId?: string }) {
  ...
  const [question, setQuestion] = useState(""); const [lastQuestion, setLastQuestion] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [draftAnswer, setDraftAnswer] = useState(""); const messagesRef = useRef<HTMLDivElement>(null);
  useEffect(() => { const container = messagesRef.current; if (!container) return; container.scrollTo({ top: container.scrollHeight, behavior: "smooth" }); }, [conversation?.messages.length, draftAnswer, loading, error]);
