...
export function PlannerModeWorkspace({ mode, schools, isMember, initialScore, initialDistrict }: { mode: "recommend" | "custom"; schools: readonly RecommendationSchool[]; isMember: boolean; initialScore?: number; initialDistrict?: string }) {
  ...
  async function addItem(school: RecommendationSchool, tier = "balanced") {
    if (items.some((item) => item.school_code === school.code)) {
    ...
    if (payload.item) {
      if (items.some((candidate) => candidate.district === payload.item?.district && candidate.school_code === payload.item?.school_code)) {
...
function CustomView({ schools, score, items, plannerState, message, onAdd, onMove, onReorder, onDelete, onSaveNotes }: { schools: readonly RecommendationSchool[]; score: number; items: readonly PlannerItem[]; plannerState: PlannerState; message: string; onAdd: (school: RecommendationSchool) => void; onMove: (index: number, direction: -1 | 1) => void; onReorder: (itemId: string, targetIndex: number) => void; onDelete: (item: PlannerItem) => void; onSaveNotes: (itemId: string, notes: string) => void }) {
  ...
  const health = analyzePlannerHealth({ serviceYear: "116", items: orderedItems.map((item) => ({ id: item.id, schoolCode: item.school_code, tier: item.tier, qualificationStatus: "unknown", hasQuota: false })) });
  return <PlannerLayout title="自己排" intro={`你的試算成績是 ${score} 分。加入並調整校科後，系統會即時進行結構化志願健檢。`}><div className="grid gap-6 lg:grid-cols-[1fr_1fr]"><section><h2 className="text-xl">我的志願順序 <span className="text-sm font-normal text-slate-500">{orderedItems.length} 校科</span></h2><p className="mt-2 text-sm leading-6 text-slate-600">可拖曳卡片或使用上下按鈕調整順位；備註會保存到同一份志願資料。</p><div className="mt-4 grid gap-3">{orderedItems.map((item, index) => <PlannerItemCard key={item.id} item={item} index={index} total={orderedItems.length} notes={plannerState.itemMeta?.[item.id]?.notes || item.notes || ""} onMove={onMove} onDrop={(targetIndex) => onReorder(item.id, targetIndex)} onDelete={onDelete} onSaveNotes={onSaveNotes} />)}{!orderedItems.length ? <p className="rounded-xl border border-dashed p-5 text-sm text-slate-500">從右側加入校科，就會出現在這裡。</p> : null}</div><PlannerHealthPanel checks={health} /></section><section><h2 className="text-xl">加入校科</h2><p className="mt-2 text-sm leading-6 text-slate-600">推薦與自己排共用同一份清單；也可以先回到找學校搜尋更完整的校科資料。</p><div className="mt-4 grid max-h-[560px] gap-2 overflow-auto">{schools.filter((school) => !selected.has(school.code)).slice(0, 30).map((school) => <SchoolRow key={`${school.code}-${school.department || "all"}`} school={school} action={() => onAdd(school)} />)}{!schools.length ? <p className="rounded-xl bg-[var(--jshs-muted-surface)] p-4 text-sm text-slate-500">目前沒有可用的推薦資料。</p> : null}</div><Link href="/schools" className="mt-4 inline-flex px-4 py-3 text-sm jshs-button-secondary">回到全國校科查詢</Link></section></div>{message ? <p className="mt-4 text-sm font-bold text-[var(--jshs-primary)]" role="status">{message}</p> : null}</PlannerLayout>;
}
...
function PlannerLayout({ title, intro, children }: { title: string; intro: string; children: React.ReactNode }) { return <><section className="jshs-hero-section"><div className="mx-auto w-[min(1120px,calc(100%-32px))] py-10"><p className="jshs-eyebrow">我的志願 · <Link href="/planner">回到我的志願</Link></p><h1 className="mt-3">{title}</h1><p className="mt-3 max-w-3xl text-base leading-7 jshs-muted-copy">{intro}</p></div></section><section className="mx-auto w-[min(1120px,calc(100%-32px))] py-8">{children}</section></>; }
