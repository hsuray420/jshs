...
function RuleAside({ rule, district, missing }: { rule: AdmissionRule; district: AdmissionDistrict; missing: readonly string[] }) { return <aside className="p-5 md:p-6 jshs-surface-card"><p className="jshs-eyebrow">規則位置感</p><h2 className="mt-2">{rule.label} · {rule.academicYear} 學年度</h2><dl className="mt-5 grid gap-3 text-sm"><MetaItem label="總分上限" value={`${rule.totalScore} 分`} /><MetaItem label="計算版本" value={`115-${district}`} /><MetaItem label="目前狀態" value="15 區皆可試算" /></dl><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">目前資料缺口</p>{missing.length ? <ul className="mt-3 grid gap-2 text-sm leading-6 text-slate-600">{missing.slice(0, 6).map((item) => <li key={item}>• {item}</li>)}{missing.length > 6 ? <li>• 還有 {missing.length - 6} 項</li> : null}</ul> : <p className="mt-3 text-sm leading-6 text-[var(--jshs-success)]">必要欄位已補齊。</p>}</div><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">同分時接著比什麼？</p><ol className="mt-3 grid gap-2 text-sm leading-6 text-slate-600">{rule.tieBreakers.slice(0, 6).map((item, index) => <li key={item}>{index + 1}. {item}</li>)}</ol></div><p className="mt-6 text-xs leading-6 jshs-muted-copy">{rule.sourceNote}</p></aside>; }
