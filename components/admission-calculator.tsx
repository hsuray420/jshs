...
function ResearchRuleForm({ rule, values, onChange }: { rule: AdmissionRule; values: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  return <div className="mt-6 grid gap-5">{(rule.fields ?? []).filter((field) => field.input_type !== "calculated" && !field.field_id.includes("exam_grade") && field.field_id !== "writing_grade").map((field) => {
    ...
    if (field.input_type === "boolean") return <label key={field.field_id} className="admission-field-group flex items-start gap-3 rounded-2xl border border-[var(--jshs-border)] p-4 text-sm font-black"><input type="checkbox" checked={Boolean(value)} onChange={(event) => onChange(field.field_id, event.target.checked)} /><span><span className="admission-field-label">{label}</span><small className="admission-field-helper">{helper}</small></span></label>;
    if (field.input_type === "radio" || field.input_type === "select" || field.input_type === "grade") return <label key={field.field_id} className="admission-field-group grid gap-2 text-sm font-black text-[var(--jshs-primary)]"><span className="admission-field-label">{label}</span><select value={value === undefined ? "" : String(value)} onChange={(event) => onChange(field.field_id, event.target.value)}><option value="">請選擇</option>{(field.options ?? []).map((option) => <option key={String(option.value)} value={String(option.value)}>{option.label}</option>)}</select><small className="admission-field-helper">{helper}</small></label>;
...
function RuleAside({ rule, missing }: { rule: AdmissionRule; missing: readonly string[] }) {
  ...
  return <aside className="p-5 md:p-6 jshs-surface-card"><p className="jshs-eyebrow">規則位置感</p><h2 className="mt-2">{rule.label} · {rule.academicYear} 學年度</h2><dl className="mt-5 grid gap-3 text-sm"><MetaItem label="總分上限" value={`${rule.totalScore} 分`} /><MetaItem label="規則資料" value="115 學年度規則" /><MetaItem label="目前狀態" value={rule.sourceId ? "規則資料已載入" : "可使用試算流程，細節請核對官方公告"} /></dl><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">目前資料缺口</p>{missing.length ? <><p className="mt-3 text-sm leading-6 text-slate-600">尚未完成／還有以下資料需要填寫：</p><ul className="mt-2 grid gap-2 text-sm leading-6 text-slate-600">{missing.slice(0, 6).map((item) => <li key={item}>• {item}</li>)}{missing.length > 6 ? <li>• 還有 {missing.length - 6} 項</li> : null}</ul></> : <p className="mt-3 text-sm leading-6 text-[var(--jshs-success)]">必要欄位已補齊。</p>}</div><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">同分比序</p><ol className="mt-3 grid gap-2 text-sm leading-6 text-slate-600">{tieBreakers.map((item, index) => <li key={`${item}-${index}`}>{index + 1}. {item}</li>)}</ol></div><details className="mt-6 border-t border-[var(--jshs-border)] pt-5"><summary className="cursor-pointer text-sm font-bold">官方資料依據</summary><p className="mt-2 text-sm leading-6 jshs-muted-copy">{rule.sourceNote}</p></details></aside>;
}
