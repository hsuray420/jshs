...
function ResearchRuleForm({ rule, values, onChange }: { rule: AdmissionRule; values: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  const fields = (rule.fields ?? []).filter((field) => field.input_type !== "calculated" && !field.field_id.includes("exam_grade") && !field.field_id.includes("exam_level") && !["exam_full_marks", "writing_grade"].includes(field.field_id));
