...
import { CHANGHUA_COMPETITION_CATALOG, getAdmissionRule, isAdmissionDistrict, type AdmissionDistrict, type AdmissionRule } from "@/lib/admission-score";
  ...
  async function calculate() {
    ...
    const response = await fetch("/api/admission/calculate", {
      ...
      body: JSON.stringify({
        ...
        ruleValues: rule.sourceId ? { ...ruleValues, preference_choices: choiceList, chinese_exam_grade: exam.chineseGrade || undefined, math_exam_grade: exam.mathGrade || undefined, english_exam_grade: exam.englishGrade || undefined, social_exam_grade: exam.socialGrade || undefined, science_exam_grade: exam.scienceGrade || undefined, writing_grade: writingLevel === "" ? undefined : Number(writingLevel) } : undefined,
      }),
    }).catch(() => null);
...
function CriteriaStep({ district, rule, criteria, updateCriteria, ruleValues, setRuleValues }: { district: AdmissionDistrict; rule: AdmissionRule; criteria: CriteriaState; updateCriteria: <Key extends keyof CriteriaState>(key: Key, value: CriteriaState[Key]) => void; ruleValues: Record<string, unknown>; setRuleValues: (value: Record<string, unknown>) => void }) {
  ...
  return <div><p className="jshs-eyebrow">比序補充</p><h2 className="mt-2">補上會影響結果的資料</h2><p className="mt-3 text-sm leading-6 jshs-muted-copy">欄位由 {rule.sourceId ? "115 學年度研究 JSON" : "目前規則摘要"} 產生；請輸入實際狀況，不要直接填換算後分數。</p>{rule.sourceId ? <ResearchRuleForm rule={rule} values={ruleValues} onChange={(key, value) => setRuleValues({ ...ruleValues, [key]: value })} /> : <><label className="mt-6 grid gap-2 text-sm font-black text-[var(--jshs-primary)]">志願序（可稍後填寫）<input value={criteria.choiceText} onChange={(event) => updateCriteria("choiceText", event.target.value)} placeholder="可先留白，之後在我的志願排序" /></label><div className="mt-7">{renderDistrictFields(district, criteria, updateCriteria, setBalanced)}</div></>}<p className="mt-6 rounded-2xl bg-[var(--jshs-muted-surface)] p-4 text-xs leading-6 jshs-muted-copy">本區總分：{rule.totalScore} 分。所有輸入只做規則試算，不等於正式審查認定。</p></div>;
}
...
function ResearchRuleForm({ rule, values, onChange }: { rule: AdmissionRule; values: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  return <div className="mt-6 grid gap-5">{(rule.fields ?? []).filter((field) => field.input_type !== "calculated" && !field.field_id.includes("exam_grade") && field.field_id !== "writing_grade").map((field) => {
    const value = values[field.field_id];
    const label = field.label;
    const helper = field.helper_text || "依研究規則填寫，系統會自動換算。";
    if (field.field_id === "competition_results") return <CompetitionField key={field.field_id} value={value} onChange={onChange} helper={helper} />;
    if (field.field_id === "fitness_session_results") return <FitnessField key={field.field_id} value={value} onChange={onChange} helper={helper} options={field.options ?? []} />;
    if (field.input_type === "boolean") return <label key={field.field_id} className="flex items-start gap-3 rounded-2xl border border-[var(--jshs-border)] p-4 text-sm font-black"><input type="checkbox" checked={Boolean(value)} onChange={(event) => onChange(field.field_id, event.target.checked)} /><span>{label}<small className="mt-1 block font-normal leading-5 jshs-muted-copy">{helper}</small></span></label>;
    if (field.input_type === "radio" || field.input_type === "select" || field.input_type === "grade") return <label key={field.field_id} className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">{label}<select value={value === undefined ? "" : String(value)} onChange={(event) => onChange(field.field_id, event.target.value)}><option value="">請選擇</option>{(field.options ?? []).map((option) => <option key={String(option.value)} value={String(option.value)}>{option.label}</option>)}</select><small className="font-normal leading-5 jshs-muted-copy">{helper}</small></label>;
    const isList = field.input_type === "multi_select" || field.input_type === "count" || (field.input_type === "hours" && field.field_id.includes("by_semester"));
    return <label key={field.field_id} className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">{label}<input type={field.input_type === "integer" || field.input_type === "hours" || field.input_type === "semester_count" ? "number" : "text"} min={field.input_type === "integer" || field.input_type === "hours" ? 0 : undefined} value={isList && Array.isArray(value) ? value.join(", ") : value === undefined ? "" : String(value)} onChange={(event) => onChange(field.field_id, isList ? event.target.value.split(/[,、，\s]+/).filter(Boolean) : (field.input_type === "integer" || field.input_type === "hours" || field.input_type === "semester_count" ? Number(event.target.value) : event.target.value))} placeholder={isList ? "以逗號分隔" : "請輸入"} /><small className="font-normal leading-5 jshs-muted-copy">{helper}</small></label>;
  })}</div>;
}
...
function RuleAside({ rule, district, missing }: { rule: AdmissionRule; district: AdmissionDistrict; missing: readonly string[] }) { return <aside className="p-5 md:p-6 jshs-surface-card"><p className="jshs-eyebrow">規則位置感</p><h2 className="mt-2">{rule.label} · {rule.academicYear} 學年度</h2><dl className="mt-5 grid gap-3 text-sm"><MetaItem label="總分上限" value={`${rule.totalScore} 分`} /><MetaItem label="計算版本" value={rule.sourceId || `115-${district}`} /><MetaItem label="目前狀態" value={rule.sourceId ? "研究 JSON 已載入" : "僅供參考，尚未拆解"} /></dl><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">目前資料缺口</p>{missing.length ? <ul className="mt-3 grid gap-2 text-sm leading-6 text-slate-600">{missing.slice(0, 6).map((item) => <li key={item}>• {item}</li>)}{missing.length > 6 ? <li>• 還有 {missing.length - 6} 項</li> : null}</ul> : <p className="mt-3 text-sm leading-6 text-[var(--jshs-success)]">必要欄位已補齊。</p>}</div><div className="mt-6 border-t border-[var(--jshs-border)] pt-5"><p className="jshs-info-group-title">同分時接著比什麼？</p><ol className="mt-3 grid gap-2 text-sm leading-6 text-slate-600">{rule.tieBreakers.slice(0, 6).map((item, index) => <li key={item}>{index + 1}. {item}</li>)}</ol></div><p className="mt-6 text-xs leading-6 jshs-muted-copy">{rule.sourceNote}</p></aside>; }
function ResultStep({ result, status, missing, rule }: { result: ScoreResult | null; status: string; missing: readonly string[]; rule: AdmissionRule }) { const rows = result ? [...rule.categories.filter((item) => item.key !== "examPerformanceScore").map((item) => [item.label, result.otherItems[item.key] ?? 0, item.max, item.description] as const), ["國中教育會考", result.exam.examPerformanceScore, rule.categories.find((item) => item.key === "examPerformanceScore")?.max ?? 0, "依五科等級與本區寫作測驗規則折算。" ] as const] : []; return <div><p className="jshs-eyebrow">結果摘要</p><h2 className="mt-2">這個結果可以怎麼用？</h2>{result ? <><div className="mt-6 flex flex-wrap items-end gap-4 rounded-2xl bg-[var(--jshs-primary)] p-6 text-white"><div><span className="block text-sm text-white/70">目前總分</span><strong className="mt-1 block text-5xl font-black text-white">{result.totalScore}</strong></div><span className="pb-2 text-sm text-white/80">滿分 {rule.totalScore} · {rule.label}</span></div><div className="mt-6 overflow-x-auto"><table><thead><tr><th>項目</th><th>目前分數</th><th>上限</th><th>怎麼來的</th></tr></thead><tbody>{rows.map(([label, score, cap, explanation]) => <tr key={String(label)}><th>{label}</th><td>{score}</td><td>{cap}</td><td>{explanation}</td></tr>)}<tr><th>會考同分比序點</th><td>{result.exam.examTotalPoints || "—"}</td><td>依本區規則</td><td>第一階段不一定加總，但會幫助同分時排序。</td></tr></tbody></table></div></> : <p className="mt-5 rounded-2xl bg-[var(--jshs-muted-surface)] p-5 text-sm leading-7 jshs-muted-copy">尚未產生結果。請回到前面補齊會考與志願序資料。</p>}<p className="mt-5 text-sm font-black text-[var(--jshs-primary)]" role="status">{status}</p><div className="mt-7 grid gap-4 md:grid-cols-2"><InfoCard title="可以用來做什麼" body="比較候選校科、看出哪一項仍可補強，並把結果帶到我的規劃。" /><InfoCard title="不能用來做什麼" body="不能當作正式錄取保證，也不能取代當年度簡章、名額、資格審查或招生委員會判定。" /></div>{missing.length ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--jshs-border)] p-4 text-sm leading-6 text-slate-600"><strong>仍有待補欄位：</strong>{missing.join("、")}</div> : null}<div className="mt-7 flex flex-wrap gap-3"><Link className="px-4 py-3 text-sm jshs-button-primary" href={`/schools?district=${districtCode(rule.code)}`}>下一步：找校科</Link><Link className="px-4 py-3 text-sm jshs-button-secondary" href={`/planner?district=${districtCode(rule.code)}`}>下一步：我的規劃</Link></div></div>; }
