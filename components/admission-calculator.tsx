...
function ContextStep({ district, academicYear, rule, calculatorAvailable, onDistrictChange, onYearChange, onLoadExample }: { district: AdmissionDistrict; academicYear: string; rule: AdmissionRule; calculatorAvailable: boolean; onDistrictChange: (value: AdmissionDistrict) => void; onYearChange: (value: string) => void; onLoadExample: () => void }) {
  return <div><p className="jshs-eyebrow">先做這件事</p><h2 className="mt-2">確認就學區與服務年度</h2><p className="mt-3 text-sm leading-6 jshs-muted-copy">每區的滿分、志願序、多元表現與會考折算都不一樣。這裡先把規則邊界固定下來。</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><label className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">就學區<select value={district} onChange={(event) => onDistrictChange(event.target.value as AdmissionDistrict)}>{districtOptions.map((option) => <option key={option.code} value={option.code}>{option.label}</option>)}</select><small className="font-normal leading-5 jshs-muted-copy">目前開放 8／15 區提供 115 官方規則參考試算；其餘區域可查詢學校與官方資料。</small></label><label className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">服務年度<select value={academicYear} onChange={(event) => onYearChange(event.target.value)}><option value="116">116 學年度</option><option value="115">115 學年度（僅來源參考）</option></select><small className="font-normal leading-5 jshs-muted-copy">{serviceYearNotice()}</small></label></div><div className="mt-5 flex flex-wrap items-center gap-3"><button type="button" onClick={onLoadExample} className="min-h-11 px-4 py-3 text-sm jshs-button-secondary">載入使用範例資料</button><span className="text-xs text-slate-500">以中投區完整示範結果，方便先了解每個項目的計算方式。</span></div><p className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">服務年度：{SERVICE_YEAR}；規則來源年度：{SOURCE_ACADEMIC_YEAR}；狀態：116 正式規則待公告。</p>{!calculatorAvailable ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--jshs-border)] p-5 text-sm leading-7 text-slate-600"><strong>此區目前不能試算，尚未開放。</strong><p className="mt-2">尚未完成規則建模，因此目前不開放試算。</p></div> : <RuleGuide rule={rule} />}</div>;
}
