...
export function NotificationCenter({ isMember }: { isMember: boolean }) {
  ...
  return <section className="mx-auto w-[min(1160px,calc(100%-32px))] py-10"><div className="p-6 md:p-8 jshs-surface-card"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="jshs-eyebrow">通知與提醒</p><h2 className="mt-2">由你決定要不要收到 LINE 通知</h2></div><span className="jshs-chip">預設全部關閉</span></div>{!isMember ? <p className="mt-5 rounded-2xl bg-[var(--jshs-muted-surface)] p-4 text-sm leading-6 jshs-muted-copy">LINE 通知需要會員身分與你的主動開通。<a className="ml-1 font-black text-[var(--jshs-primary)] underline" href="/api/line/login/start">使用 LINE 登入</a></p> : null}<div className="mt-6 grid gap-3">{settings.map(([id, title, detail]) => <label id={id} key={id} className={`flex items-center gap-4 rounded-2xl bg-[var(--jshs-muted-surface)] p-4 ${!isMember ? "opacity-60" : "cursor-pointer"}`}><input type="checkbox" checked={Boolean(enabled[id])} disabled={!isMember || Boolean(saving)} onChange={() => void toggle(id)} className="h-5 w-5" /><span className="min-w-0"><strong className="block">{title}</strong><span className="mt-1 block text-sm leading-6 jshs-muted-copy">{detail}</span></span><span className="ml-auto text-xs font-black text-[var(--jshs-primary)]">{enabled[id] ? "已開通" : "未開通"}</span></label>)}</div>{status ? <p className="mt-5 text-sm font-bold text-[var(--jshs-success)]" role="status">{status}</p> : null}<p className="mt-3 text-xs leading-5 jshs-muted-copy">後台仍可暫停任何一類通知；即使後台開啟，也必須先由你在這裡開通才會推送。</p></div></section>;
}
