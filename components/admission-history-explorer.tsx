  ...
  return <>
    ...
    <section className="mx-auto w-[min(1180px,calc(100%-32px))] py-8 md:py-12">
      <div className="grid gap-4 p-5 jshs-surface-card md:grid-cols-[1fr_220px_180px] md:p-7">
        <label className="grid gap-2 text-sm font-black text-[var(--jshs-primary)]">搜尋學校、科別、縣市或代碼<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例如：中科實驗、普通科、060323" /></label>
...
function HistoryGroup({ title, sourceType, schools }: { title: string; sourceType: HistoricalSourceType; schools: readonly HistoryRecord[] }) {
  return <section aria-labelledby={`history-${sourceType}`}><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="jshs-eyebrow">{historicalSourceLabel(sourceType)}</p><h2 id={`history-${sourceType}`} className="mt-2 text-2xl font-black">{title}</h2></div><span className="text-sm jshs-muted-copy">{schools.length} 筆</span></div>{schools.length ? <div className="mt-4 grid gap-4 lg:grid-cols-2">{schools.map((school) => <article key={`${school.districtCode}:${school.code}`} className="p-5 jshs-surface-card"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[.12em] text-[var(--jshs-primary)]">{school.districtLabel} · {school.code}</p><h3 className="mt-2 text-xl font-black">{school.name}</h3></div><span className="jshs-chip">{historicalSourceLabel(sourceType)}</span></div><p className="mt-3 text-sm leading-6 jshs-muted-copy">{[school.city, school.area, school.program].filter(Boolean).join(" · ")}</p><dl className="mt-4 grid gap-3 sm:grid-cols-2"><HistoryFact label="最低錄取成績" value={school.referenceScore} /><HistoryFact label="成績年度" value={school.scoreYear || "未標示"} /></dl><p className="mt-4 rounded-2xl bg-[var(--jshs-muted-surface)] p-4 text-sm leading-7 text-slate-700">科別／校科：{school.departmentsRaw || "未標示"}</p><p className="mt-4 text-xs leading-5 text-slate-500">來源：{school.sourceName}；{school.sourceNote || "CSV 未補充來源備註"}</p><Link className="mt-4 inline-block text-sm font-black text-[var(--jshs-primary)]" href={`/schools/${school.districtCode}/${school.code}#alumni`}>查看學校詳情與學長姐分享 →</Link></article>)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-[var(--jshs-border)] p-6 text-sm leading-6 jshs-muted-copy">目前沒有符合條件的{historicalSourceLabel(sourceType)}，不以推估資料補空白。</div>}</section>;
}
